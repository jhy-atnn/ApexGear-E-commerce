...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nav-badge { margin-left: auto; background: var(--danger); color: #fff; font-size: .6rem; font-weight: 700; padding: 2px 6px; border-radius: 20px; }
        ...
        .status-badge.pending  { background: rgba(245,197,24,.15); color: #8a6d00; }
        .status-badge.process  { background: rgba(0,194,255,.12); color: #007aad; }
        .status-badge.shipped  { background: rgba(11,47,168,.1); color: var(--blue); }
        .status-badge.delivered{ background: rgba(0,214,143,.12); color: #00835a; }
        .status-badge.canceled { background: rgba(255,59,92,.1); color: var(--danger); }

        /* Payment status badges */
        .pay-badge { display: inline-flex; align-items: center; gap: 5px; padding: 3px 10px; border-radius: 20px; font-size: .72rem; font-weight: 700; }
        .pay-badge.success  { background: rgba(0,214,143,.12); color: #00835a; }
        .pay-badge.pending  { background: rgba(245,197,24,.15); color: #8a6d00; }
        .pay-badge.failed   { background: rgba(255,59,92,.1); color: var(--danger); }

        /* Order card */
        .order-card { background: var(--card-bg); border: 1px solid var(--border); border-radius: 14px; padding: 28px 28px 8px; margin-bottom: 24px; }

        /* Progress stepper */
        .progress-stepper { display: flex; align-items: center; gap: 0; margin: 6px 0 2px; }
        .step { display: flex; flex-direction: column; align-items: center; flex: 1; position: relative; }
        .step:not(:last-child)::after { content: ''; position: absolute; top: 11px; left: 55%; width: 90%; height: 2px; background: var(--border); z-index: 0; }
        .step:not(:last-child).done::after { background: var(--success); }
        .step-dot { width: 22px; height: 22px; border-radius: 50%; border: 2px solid var(--border); background: #fff; z-index: 1; display: flex; align-items: center; justify-content: center; font-size: .55rem; color: var(--text-muted); }
        .step.done .step-dot { background: var(--success); border-color: var(--success); color: #fff; }
        .step.active .step-dot { background: var(--accent); border-color: var(--accent); color: var(--sidebar-bg); }
        .step-lbl { font-size: .6rem; font-weight: 600; color: var(--text-muted); margin-top: 4px; text-align: center; white-space: nowrap; }
        .step.done .step-lbl, .step.active .step-lbl { color: var(--text-main); }

        /* Order table */
        .order-table { overflow-x: auto; }
        .order-table table { width: 100%; border-collapse: collapse; }
        .order-table th { background: #f8fafd; padding: 10px 14px; font-size: .72rem; font-weight: 700; text-transform: uppercase; letter-spacing: .07em; color: var(--text-muted); border-bottom: 1px solid var(--border); white-space: nowrap; }
        .order-table td { padding: 14px 14px; border-bottom: 1px solid var(--border); vertical-align: middle; font-size: .85rem; }
        .order-table tr:last-child td { border-bottom: none; }
        .order-table tr:hover td { background: rgba(0,194,255,.03); }

        .ref-code { font-family: 'Barlow Condensed', monospace; font-weight: 800; font-size: .95rem; color: var(--blue); letter-spacing: .04em; }

        /* View details button */
        .btn-view { display: inline-flex; align-items: center; gap: 5px; padding: 5px 11px; border-radius: 7px; font-size: .75rem; font-weight: 600; cursor: pointer; border: 1.5px solid rgba(11,47,168,.25); color: var(--blue); background: rgba(11,47,168,.05); text-decoration: none; transition: all .15s; }
        ...
        .modal-header-custom { padding: 18px 24px; background: var(--sidebar-bg); color: #fff; display: flex; align-items: center; justify-content: space-between; }
        .modal-header-custom .modal-title { font-family: 'Barlow Condensed', sans-serif; font-weight: 800; font-size: 1.2rem; text-transform: uppercase; letter-spacing: .04em; }
        .modal-header-custom .btn-close { filter: invert(1); opacity: .6; }
        .modal-body-custom { padding: 24px; background: #fff; max-height: 75vh; overflow-y: auto; }
        .modal-footer-custom { padding: 14px 24px; background: #f8fafd; border-top: 1px solid var(--border); display: flex; justify-content: flex-end; gap: 10px; }

        /* Detail grid inside modal */
        .detail-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px 20px; margin-bottom: 18px; }
        .detail-item label { font-size: .68rem; font-weight: 700; text-transform: uppercase; letter-spacing: .07em; color: var(--text-muted); display: block; margin-bottom: 3px; }
        .detail-item span { font-size: .9rem; font-weight: 600; color: var(--text-main); }
        .section-divider { border: none; border-top: 1px solid var(--border); margin: 16px 0; }

        /* Order items list */
        .items-list { list-style: none; padding: 0; margin: 0; }
        .items-list li { display: flex; justify-content: space-between; align-items: center; padding: 8px 12px; border-radius: 8px; margin-bottom: 6px; background: #f8fafd; font-size: .84rem; }
        .items-list li .item-name { font-weight: 600; }
        .items-list li .item-price { color: var(--blue); font-weight: 700; }

        /* Payment section */
        .payment-box { background: #f8fafd; border: 1px solid var(--border); border-radius: 10px; padding: 16px; margin-top: 12px; }
        .payment-box .pay-row { display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; font-size: .84rem; }
        .payment-box .pay-row:last-child { margin-bottom: 0; }
        .payment-box .pay-label { color: var(--text-muted); font-weight: 600; }
        .payment-box .pay-value { font-weight: 700; }
        .screenshot-thumb { width: 100%; max-width: 260px; border-radius: 8px; border: 1px solid var(--border); margin-top: 10px; cursor: pointer; }
        .privacy-note { font-size: .72rem; color: var(--text-muted); display: flex; align-items: center; gap: 5px; margin-top: 8px; }

        /* Remarks textarea */
        .remarks-field { width: 100%; border: 1.5px solid var(--border); border-radius: 8px; padding: 8px 12px; font-size: .85rem; font-family: 'Barlow', sans-serif; resize: vertical; color: var(--text-main); }
        .remarks-field:focus { outline: none; border-color: var(--blue); box-shadow: 0 0 0 3px rgba(11,47,168,.1); }
