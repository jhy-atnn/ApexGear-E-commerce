...
// We keep Inventory just to fetch the pending orders count for the sidebar badge
$inv = new Inventory();
...
// ── SEARCH & FETCH ACTIVE USERS (Using image_path) ──
$search = trim($_GET['q'] ?? '');
$query = "
...
while ($row = $result->fetch_assoc()) {
    // Also fetch their latest shipping address if any
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .avatar { width: 38px; height: 38px; border-radius: 50%; background: linear-gradient(135deg, var(--blue), var(--accent)); display: flex; align-items: center; justify-content: center; color: #fff; font-weight: 800; font-size: .88rem; flex-shrink: 0; overflow: hidden; }
        .user-name { font-weight: 600; }
        .user-meta { font-size: .75rem; color: var(--text-muted); }

        .role-pill { display: inline-block; padding: 3px 10px; border-radius: 20px; font-size: .7rem; font-weight: 700; text-transform: uppercase; background: rgba(0,194,255,.1); color: var(--accent); }
        ...
        .btn-archive { background: rgba(245,197,24,.1); color: #8a6d00; border: 1px solid rgba(245,197,24,.3); }
        .btn-archive:hover { background: #f5c518; color: #000; }
    ...
    <main class="page-body">
        ...
        <div class="stat-row">
            <div class="stat-card">
                ...
                <div><div class="stat-val"><?php echo $userCount; ?></div><div class="stat-lbl">Active Users</div></div>
            </div>
            <div class="stat-card">
                <div class="stat-icon cyan"><i class="fas fa-map-marker-alt"></i></div>
                <div><div class="stat-val"><?php echo count(array_filter($users, fn($u) => !empty($u['shipping']))); ?></div><div class="stat-lbl">With Shipping Data</div></div>
            </div>
        </div>
