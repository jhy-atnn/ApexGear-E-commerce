...
$onSale = array_filter($products, fn($p) =>
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* Sidebar */
        .sidebar { width: var(--sidebar-w); min-height: 100vh; background: var(--sidebar-bg); display: flex; flex-direction: column; position: fixed; top: 0; left: 0; bottom: 0; z-index: 1000; }
        .sidebar-brand { display: flex; align-items: center; gap: 10px; padding: 26px 24px 20px; border-bottom: 1px solid rgba(255,255,255,.07); text-decoration: none; }
        .sidebar-brand img { height: 34px; }
        .t1 { font-family: 'Barlow Condensed', sans-serif; font-weight: 900; font-size: 1.45rem; color: #fff; }
        .t2 { font-family: 'Barlow Condensed', sans-serif; font-weight: 900; font-size: 1.45rem; color: var(--accent); }
        .sidebar-badge { margin-left: auto; background: var(--accent); color: var(--sidebar-bg); font-size: .6rem; font-weight: 800; letter-spacing: .06em; padding: 2px 7px; border-radius: 4px; text-transform: uppercase; }
        .sidebar-section-label { font-size: .62rem; font-weight: 700; letter-spacing: .12em; text-transform: uppercase; color: rgba(255,255,255,.25); padding: 20px 24px 8px; }
        .sidebar-nav { flex: 1; }
        .sidebar-nav a { display: flex; align-items: center; gap: 13px; padding: 11px 24px; color: rgba(255,255,255,.58); text-decoration: none; font-size: .88rem; font-weight: 500; border-left: 3px solid transparent; transition: all .18s; }
        .sidebar-nav a:hover { background: var(--sidebar-hover); color: #fff; }
        .sidebar-nav a.active { background: rgba(0,194,255,.12); color: var(--accent); border-left-color: var(--accent); font-weight: 600; }
        .sidebar-nav a i { width: 18px; text-align: center; }
        .sidebar-footer { padding: 16px 24px; border-top: 1px solid rgba(255,255,255,.07); }
        .sidebar-footer a { display: flex; align-items: center; gap: 10px; color: rgba(255,255,255,.4); font-size: .82rem; text-decoration: none; }
        .sidebar-footer a:hover { color: #fff; }

        /* Layout */
        .main-wrap { margin-left: var(--sidebar-w); flex: 1; display: flex; flex-direction: column; min-height: 100vh; }
        .topbar { background: var(--card-bg); border-bottom: 1px solid var(--border); padding: 0 32px; height: 64px; display: flex; align-items: center; justify-content: space-between; position: sticky; top: 0; z-index: 100; }
        .topbar-title { font-family: 'Barlow Condensed', sans-serif; font-weight: 800; font-size: 1.5rem; text-transform: uppercase; }
        .page-body { padding: 28px 32px 48px; flex: 1; }

        /* Tabs */
        .tab-nav { display: flex; gap: 0; border-bottom: 2px solid var(--border); margin-bottom: 24px; }
        .tab-btn { padding: 10px 22px; font-family: 'Barlow Condensed', sans-serif; font-weight: 700; font-size: 1rem; text-transform: uppercase; letter-spacing: .05em; background: none; border: none; border-bottom: 3px solid transparent; margin-bottom: -2px; color: var(--text-muted); cursor: pointer; transition: all .18s; }
        .tab-btn.active { color: var(--blue); border-bottom-color: var(--blue); }
        .tab-pane { display: none; }
        .tab-pane.active { display: block; }

        /* Panel */
        .panel { background: var(--card-bg); border: 1px solid var(--border); border-radius: 14px; overflow: hidden; margin-bottom: 24px; }
        ...
        .panel-title { font-family: 'Barlow Condensed', sans-serif; font-weight: 800; font-size: 1.1rem; text-transform: uppercase; letter-spacing: .04em; }
        .panel-body { padding: 22px 24px; }

        /* Form fields */
        .field-group { margin-bottom: 14px; }
        .field-row { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; }
        .field-label { display: block; font-size: .72rem; font-weight: 700; text-transform: uppercase; letter-spacing: .07em; color: var(--text-muted); margin-bottom: 5px; }
        .field-control { width: 100%; padding: 9px 12px; border: 1.5px solid var(--border); border-radius: 8px; font-size: .88rem; font-family: 'Barlow', sans-serif; color: var(--text-main); background: #fff; outline: none; transition: border .18s; }
        .field-control:focus { border-color: var(--blue); box-shadow: 0 0 0 3px rgba(11,47,168,.1); }
        .btn-submit { width: 100%; padding: 11px; background: var(--blue); color: #fff; border: none; border-radius: 9px; font-family: 'Barlow Condensed', sans-serif; font-size: 1.05rem; font-weight: 800; text-transform: uppercase; letter-spacing: .05em; cursor: pointer; margin-top: 4px; transition: background .15s; }
        .btn-submit:hover { background: #0928a0; }
        .radio-group { display: flex; gap: 18px; margin-top: 4px; }
        .radio-item { display: flex; align-items: center; gap: 6px; font-size: .84rem; cursor: pointer; }

        /* Table */
        .inv-table { width: 100%; border-collapse: collapse; }
        .inv-table thead th { background: #f8fafd; padding: 10px 14px; font-size: .72rem; font-weight: 700; text-transform: uppercase; letter-spacing: .07em; color: var(--text-muted); border-bottom: 1px solid var(--border); white-space: nowrap; }
        .inv-table tbody td { padding: 14px 14px; border-bottom: 1px solid var(--border); vertical-align: middle; }
        .inv-table tbody tr:last-child td { border-bottom: none; }
        .inv-table tbody tr:hover td { background: rgba(0,194,255,.025); }

        .product-thumb { width: 40px; height: 40px; border-radius: 8px; overflow: hidden; background: #f0f3f9; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
        .product-thumb img, .product-thumb svg { width: 100%; height: 100%; object-fit: cover; }
        .product-name { font-size: .88rem; font-weight: 600; }
        .product-meta { font-size: .7rem; font-weight: 600; color: var(--text-muted); text-transform: uppercase; letter-spacing: .05em; margin-top: 2px; }
        .price-tag { font-weight: 700; color: var(--blue); font-size: .9rem; }

        .stock-badge { display: inline-block; padding: 3px 10px; border-radius: 20px; font-size: .72rem; font-weight: 700; }
        .stock-badge.ok   { background: rgba(0,214,143,.12); color: #00a870; }
        .stock-badge.low  { background: rgba(255,59,92,.12); color: var(--danger); }
        .stock-badge.zero { background: rgba(107,122,153,.12); color: var(--text-muted); }

        .sale-pill { display: inline-block; background: rgba(255,59,92,.1); color: var(--danger); font-size: .68rem; font-weight: 800; padding: 2px 7px; border-radius: 10px; }
        .shipping-pill { display: inline-block; background: rgba(0,194,255,.1); color: #007aad; font-size: .68rem; font-weight: 700; padding: 2px 7px; border-radius: 10px; }

        .act-btn { display: inline-flex; align-items: center; gap: 5px; padding: 5px 11px; border-radius: 7px; font-size: .75rem; font-weight: 600; cursor: pointer; border: 1.5px solid transparent; transition: all .15s; text-decoration: none; }
        .act-btn-edit { border-color: rgba(11,47,168,.3); color: var(--blue); background: rgba(11,47,168,.05); }
        .act-btn-edit:hover { background: var(--blue); color: #fff; border-color: var(--blue); }
        .act-btn-archive { border-color: rgba(245,197,24,.35); color: #8a6d00; background: rgba(245,197,24,.07); }
        .act-btn-archive:hover { background: #f5c518; color: #000; border-color: #f5c518; }

        /* Deals cards */
        .deals-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 16px; }
        .deal-card { background: #fff; border: 1px solid var(--border); border-radius: 12px; overflow: hidden; position: relative; }
        .deal-card .deal-badge { position: absolute; top: 10px; right: 10px; background: var(--danger); color: #fff; font-size: .7rem; font-weight: 800; padding: 3px 9px; border-radius: 8px; }
        .deal-card .deal-body { padding: 14px; }
        .deal-card .deal-name { font-weight: 700; font-size: .88rem; margin-bottom: 4px; }
        .deal-card .deal-price { color: var(--blue); font-weight: 800; }
        .deal-card .deal-sale-price { color: var(--danger); font-size: .82rem; }
        .deal-card .deal-expiry { font-size: .7rem; color: var(--text-muted); margin-top: 4px; }
        .featured-crown { color: #f5c518; font-size: .85rem; }

        /* Alert */
        .apex-alert { display: flex; align-items: center; gap: 12px; padding: 13px 18px; border-radius: 10px; font-size: .88rem; font-weight: 500; margin-bottom: 20px; position: relative; }
        .apex-alert.success { background: rgba(0,214,143,.1); border-left: 4px solid var(--success); color: #007a52; }
        .apex-alert.info { background: rgba(11,47,168,.08); border-left: 4px solid var(--blue); color: var(--blue); }
        .apex-alert.warn { background: rgba(245,197,24,.1); border-left: 4px solid #f5c518; color: #8a6d00; }
        .apex-alert .close-btn { position: absolute; right: 14px; top: 50%; transform: translateY(-50%); background: none; border: none; font-size: .9rem; color: inherit; opacity: .6; cursor: pointer; }

        /* Modal */
        .modal-content { border: none; border-radius: 14px; overflow: hidden; box-shadow: 0 20px 60px rgba(0,0,0,.18); }
        .modal-header-custom { padding: 20px 26px; background: var(--sidebar-bg); color: #fff; display: flex; align-items: center; justify-content: space-between; }
        .modal-header-custom .modal-title { font-family: 'Barlow Condensed', sans-serif; font-weight: 800; font-size: 1.3rem; text-transform: uppercase; letter-spacing: .04em; }
        .modal-header-custom .btn-close { filter: invert(1); opacity: .6; }
        .modal-body-custom { padding: 26px; background: #fff; }
        .modal-footer-custom { padding: 16px 26px; background: #f8fafd; border-top: 1px solid var(--border); display: flex; justify-content: flex-end; gap: 10px; }
        .btn-cancel { padding: 9px 20px; border: 1.5px solid var(--border); border-radius: 8px; background: #fff; color: var(--text-muted); font-weight: 600; font-size: .85rem; cursor: pointer; }
        .btn-save { padding: 9px 24px; border: none; border-radius: 8px; background: var(--blue); color: #fff; font-family: 'Barlow Condensed', sans-serif; font-size: 1rem; font-weight: 800; text-transform: uppercase; cursor: pointer; }
        .btn-save:hover { background: #0928a0; }
                        ...
                        <div class="field-row">
                            <div class="field-group">
                                ...
                                <select name="brand" class="field-control" required>
                                    ...
                                    <option>Apple</option><option>ASUS</option><option>Corsair</option>
                                    <option>Dell</option><option>HP</option><option>Lenovo</option>
                                    <option>Intel</option><option>Logitech</option><option>NVIDIA</option>
                                    <option>Samsung</option><option>Sony</option><option>Razer</option>
                                </select>
                            </div>
                            <div class="field-group">
                                ...
                                <select name="category" class="field-control" required>
                                    ...
                                    <option value="laptop">Laptop</option><option value="desktop">Desktop / PC</option>
                                    <option value="tablet">Tablet</option><option value="phone">Phone</option>
