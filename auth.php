...
async function doLogin() {
    ...
    if (!u || !p) { showAlert('loginAlert', 'Please fill in both fields.'); return; }
    hideAlert('loginAlert');
    setLoading('loginBtn', true);
    const r = await apexPost({ action: 'login', username: u, password: p });
...
async function doRegister() {
    ...
    if (!u || !e || !p) { showAlert('registerAlert', 'All fields are required.'); return; }
    hideAlert('registerAlert');
    setLoading('registerBtn', true);
    const r = await apexPost({ action: 'register', username: u, email: e, password: p });
...
async function doVerify() {
    ...
    if (code.length < 6) { showAlert('verifyAlert', 'Please enter the full 6-digit code.'); return; }
    hideAlert('verifyAlert');
    setLoading('verifyBtn', true);
    const r = await apexPost({ action: 'verify', code });
...
async function doSocial(provider) {
    const r = await apexPost({ action: 'social', provider });
    if (r.success) {
